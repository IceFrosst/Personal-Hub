...
| `news.google.com/rss/search` | ✅ Live (New tab only) | Press mentions of hackathons in the Baltics, Poland and the Nordics — **English and Lithuanian words only** (Ignas's rule: if it can't be found in either, English won't be the event's language). Five queries (`hakatonas`; `hackathon` + LT / LV-EE / PL / Nordic city names). Implemented in `lib/ingest/newsrss.ts`; live 2026-09-15: **3 announcements in the last 45 days** — Cyber Arena šaulių hakatonas (Delfi), University of Latvia EUDIS Defence Hackathon 2026 (Labs of Latvia), Google's Warsaw student AI hackathon (EdTech Innovation Hub) — none of which any other source carries. **Limit, by construction:** Google wraps every link in an opaque `news.google.com/rss/articles/<token>` redirect that nothing server-side can resolve (see the resolver post-mortem below), so rows have **no dates** and never enter the main feed; they appear in the **New tab for 72 h** as a click-through stream, and enrichment skips them. Retrospectives ("vykęs", "praūžė", "winners") and items older than 45 days are dropped. |
...
| newsrss | 5 queries × 1 RSS page, MAX_PER_QUERY 15 | Google News caps a search feed at ~100 items; 45-day window binds first | ✅ n/a |
