...
| `hackathonhub.eu` | ✅ Live | Curated European directory (DACH-first), asked for by Ignas 2026-09-16. No JSON API and the `/events` page is an app shell — but every event has a **Markdown twin** at `/events/<slug>.md` with YAML front matter (title, date, end_date, location "City, CC", format, type, level, prize, tags, organiser `url`) and `/sitemap-events.xml` lists all 509 pages. Implemented in `lib/ingest/hackathonhub.ts`: sitemap → drop slugs with a past year suffix → fetch `.md` (concurrency 8) → keep type hackathon/gamejam or hack-titled → future start. Live 2026-09-16: **130 upcoming rows in 34 s, 102 new to the catalog** — Germany 27, Italy 12, Spain 11, Switzerland 11, France 10, Netherlands 9, UK 14, plus Latvia, Estonia, Finland, Poland, Malta, Croatia, Bulgaria. Rows carry the **organiser's URL** (Luma links normalised `luma.com` → `lu.ma`) so they merge with Luma/Eventbrite rows instead of doubling — 28 of 130 already matched. Supplies exact dates and format; no registration deadline, so rows wait on enrichment like Eventbrite's. |
...
- **Hackathon Hub** (`lib/ingest/hackathonhub.ts`, unit test `test/hackathonhub.test.ts`).
  ...
  aggregator we read had. The `.md` twins are the whole trick; if they vanish the source
  throws ("0 of N .md pages fetched") rather than returning an empty sweep.
