...
**Latest pass — group-photo question moved out of the opening rotation:**
...
- The question is now path-specific: BUSINESS asks it after the proposal and before `/appointment`; SIDEQUEST asks it after the idea and before the supplies screen. DATE and SPECIAL never receive it.
- `components/visa-steps/GroupPhotoQuestion.tsx` is the shared renderer. BUSINESS and SIDEQUEST resume at the first incomplete screen after refresh/back-navigation. The answer continues to use the existing audited/persisted `screeningQuestion`/`screeningAnswer` fields, so no schema change was needed.

**Previous pass — production abandoned-draft audit INSERT fix and bounded archive cleanup:**
...
- **Thumbnail restore priority, and a same-device rescue:** `mapSubmittedApplication` now uses the record's OWN `selfieThumbnailUrl` directly when present. Only when the record has none does `applicationContext.tsx#restoreSubmittedApplication` fall back to the CURRENT session's in-memory thumbnail via the existing exact-reference-code-match guard, `resolveRestoredThumbnail` (unchanged: any mismatch or missing reference code yields no thumbnail — the placeholder frame, never a stranger's/stale photo). When that fallback DOES resolve a thumbnail, it's now also rescued into the matching row of THIS DEVICE'S localStorage application log via a new narrowly scoped `lib/api.ts#persistApplicationThumbnail` (exact `referenceCode` match only, no-op otherwise — its actual merge logic is the pure, directly-unit-tested `lib/applicationState.ts#mergeSelfieThumbnail`), so a photo that was previously only "safe for this tab's remaining session" now survives even after that tab/session eventually closes.
- Restore still shows `/visa-issued` directly (no route through `/appointment`/calendar), still forward-locked/read-only, and does not fetch private Storage or a public photo endpoint — it uses the small on-file/session thumbnail. Private Storage upload remains a separate processing concern: the review image can come from the thumbnail when full-resolution capture is unavailable.
- **New/updated tests** (`test/applicationState.test.mjs`, new `test/api.test.mjs`): legacy-complete records now synthesize a truthy, deterministic, non-regenerating `serial`/`issuedDate` (including UTC timezone invariance and `DATE ON FILE` missing/invalid fallbacks); genuinely-incomplete records (missing slot/referenceCode/selfie) still correctly get `null`; an existing serial/issuedDate is never overwritten; a record's own `selfieThumbnailUrl` is used directly; `mergeSelfieThumbnail`'s exact-match/no-match/already-set no-op cases; and (via source-text assertions on `lib/api.ts` and `app/processing/page.tsx`, the same convention `test/migrationPayloadGuards.test.mjs` already uses for SQL — these modules' runtime imports don't resolve under plain Node ESM) that the `republic.applications` table JSON omits `selfieThumbnailUrl`/`submittedAt`, local-log write keeps them while stripping `intel`/`draftId`, and processing retains private Storage's thumbnail review-image fallback.

**Previous pass — same-device final application restore:** the returning-application card now has a prominent `VIEW FINAL APPLICATION` action. It maps the local `ApplicationRecord` through `lib/applicationState.ts` into a complete, forward-locked read-only state and routes to `/visa-issued`; it restores persisted fields and selfie metadata without raw-photo fetching. The current session's `selfieThumbnailUrl` is carried over only when `lib/applicationState.ts#resolveRestoredThumbnail` confirms the session's `referenceCode` exactly matches the record being restored — any mismatch or missing reference code drops the thumbnail so `/visa-issued` falls back to its "PHOTO ON FILE" placeholder rather than ever risking showing the wrong application's photo. Fresh sessions with a local application do not mint or audit a throwaway draft during provider hydration, and the restore path itself (`applicationContext.tsx#restoreSubmittedApplication`) never mints/records one either — `mapSubmittedApplication` always sets `draftId: null`. Mapper tests cover all visa-specific fields, legacy records, completion locks, and `resolveRestoredThumbnail`'s match/mismatch/missing-reference cases.
...
Verified in this pass: `npm test` (86 tests), `npm run typecheck`, `npm run build`,
