...
**Uncommitted on branch `claude/zealous-rubin-J8egx` (built, `next build` passes — not yet deployed):** the suggestion is a **quiet, boxless "Suggested tasks" list near the top of the gate** (no card/border/dividers — grouped by space and type), showing **up to two** AI-picked active tasks as **title + a small priority dot + due date**. Priority is a 6px dot (high=coral, medium=amber, low=neutral) rather than colored words, so the gate's gradient wordmark stays the only loud colour. Scoring weighs priority + due date + time of day and is **gentle late at night**; the same logic drives the heuristic fallback, and Gemini returns task IDs only (priority/date are read from the row). The earlier popup + boxed-card presentations were dropped. `?demo=1` still renders sample data and must be removed before production.
